...
video_files = os.listdir(VIDEO_DIR).sort()
opening_files = os.listdir(OPENING_DIR).sort()
closing_files = os.listdir(CLOSING_DIR).sort()
...
# TODO make it argument
VIDEO_TO_EDIT = os.path.join(VIDEO_DIR, '20170309 0450 - Polsat Jim Jam - Dinopociąg_ odc_35.ts')
...
def analyse_frames_opening(path, STARTING_FRAME, BEGIN_FRAME):
    ...
    for fn in res_files:
        ...
        if float(cmp[0]) < 4000:
            ...
            if frms > 100 or FOUND and FOUND_NO > 2:
                break
        frms = frms + 1
    begin_frame = min(zip(found_frames))[0][1]
    print('###', begin_frame)
    begin_frame = get_frame_number(begin_frame)
...
seek = analyse_frames_opening(os.path.join('/',
                                    'home',
                                    'fasolens',
                                    'Videos',
                                    'dinopociag',
